...
export default {
  ...
  methods: {
    ...
    specialFun(values) {
      ...
      if (
        (this.dataList.currTaskDefinitionKey === 'SealManager' && this.dataList.currFlowId === 'PurchaseApprove') || // 采购 印章管理员-最终节点
        (this.dataList.currTaskDefinitionKey === 'Dept' &&
          this.$refs.detail.changePurchaseId &&
          (this.$refs.detail.hasNew.length <= 0 || this.$refs.detail.isSeal.indexOf('1') === -1)) // 采购-变更-不传附件-事业部经理审批-最终节点
      ) {
