...
<script>
import { projptenderpreqpayList } from '../../api/bidManagement'
...
export default {
  ...
  created () {
    projptenderpreqpayList(this.dataList.dataId).then(res => {
      if (res.data) {
        this.custList = res.data
        this.custList.amountWords = this.intToChinese(this.custList.amount)
        this.custList.recordRecordNo = this.custList.recordno.substring(this.custList.recordno.length - 4)
